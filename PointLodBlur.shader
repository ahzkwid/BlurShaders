    ...
    Properties
    {
        ...
        SubShader
        {
            ...
            Pass
            {
                ...
                fixed4 tex2DlodPoint(sampler2D _sampler, float4 _pos)
                {
                    int _BoardSize = _MainTex_TexelSize.z / pow(2, _pos.w);
                    return tex2D(_MainTex, (floor(_pos.xy * _BoardSize)) / _BoardSize);
                }
                fixed4 tex2DlodLinear(sampler2D _sampler, float4 _pos)
                {
                    int _BoardSize = _MainTex_TexelSize.z / pow(2, _pos.w);
                    float4 col = tex2DlodPoint(_sampler, float4(_pos.xy, _pos.zw));
                    float4 col2 = tex2DlodPoint(_sampler, float4(_pos.xy + float2(1, 0) / _BoardSize, _pos.zw));
                    float4 col3 = tex2DlodPoint(_sampler, float4(_pos.xy + float2(0, 1) / _BoardSize, _pos.zw));
                    float4 col4 = tex2DlodPoint(_sampler, float4(_pos.xy + float2(1, 1) / _BoardSize, _pos.zw));
                ...
                {
                    return tex2DlodLinear(_sampler, _pos);
                }
                ...
                fixed4 lodBlurTexel(float blurStrength, float2 uv)
                {
                    ...
                    fixed radius = pow(2, blurStrength- 1);
                ...
                */
                ...
                if (blurStrength>0)
                {
                   ...
                   // detail = clamp(detail, 1 , blurStrength);
                    ...
                    {
                        col += lodBlurTexelSimple(blurStrength + j, i.uv);
                    }
